...
const DosageList = ({ showToast }) => {
  ...
  const fetchDosageForms = async () => {
    try {
      ...
      if (typeof showToast === "function") {
        showToast("Failed to fetch dosage forms", "error");
      }
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (typeof showToast === "function") {
        showToast("Dosage form deleted successfully!");
      }
      fetchDosageForms();
    } catch (err) {
      const errorMessage =
  ...
  const handleSave = () => {
    ...
    fetchDosageForms();
    if (typeof showToast === "function") {
      showToast("Dosage form saved successfully!");
    }
  };

  // Pagination logic
  const totalPages = Math.ceil(dosageForms.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentDosageForms = dosageForms.slice(
    startIndex,
  ...
  }, [dosageForms, totalPages, currentPage]);
